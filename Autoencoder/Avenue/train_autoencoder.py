...
def convnext_autoencoder():
    ...
    decoder = nn.Sequential(
        nn.ConvTranspose2d(768, 512, 4, 2, 1), nn.ReLU(True),  # 8x8
        nn.ConvTranspose2d(512, 256, 4, 2, 1), nn.ReLU(True),  # 16x16
        nn.ConvTranspose2d(256, 128, 4, 2, 1), nn.ReLU(True),  # 32x32
        nn.ConvTranspose2d(128, 64, 4, 2, 1), nn.ReLU(True),   # 64x64
        nn.ConvTranspose2d(64, 3, 4, 2, 1), nn.Tanh()           # 128x128
    )
...
def train_model(model, loader, epochs=50, lr=1e-3):
    model.to(device)
    # optimizer only updates decoder parameters
