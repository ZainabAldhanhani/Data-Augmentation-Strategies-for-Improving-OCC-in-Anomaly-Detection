...
class CutPasteTransform:
    def __init__(self, patch_ratio=0.2, probability=0.5):
    ...
    def __call__(self, img: Image.Image) -> Image.Image:
        ...
        arr = np.array(img)
        h, w = arr.shape
        ph, pw = int(h * self.patch_ratio), int(w * self.patch_ratio)
...
def load_ucsd_gt(root):
    ...
    if not m_files:
        raise FileNotFoundError(f"No .m files found in {os.path.join(root,'Test')}")
