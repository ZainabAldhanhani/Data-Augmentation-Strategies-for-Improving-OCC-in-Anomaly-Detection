...
# Elastic deformation augmentation class
class ElasticTransform:
    ...
    def __call__(self, img: Image.Image) -> Image.Image:
        if random.random() > self.probability:
            return img
        arr = np.array(img)
        dx = ndimage.gaussian_filter((np.random.rand(*arr.shape) * 2 - 1), self.sigma) * self.alpha
        dy = ndimage.gaussian_filter((np.random.rand(*arr.shape) * 2 - 1), self.sigma) * self.alpha
        x, y = np.meshgrid(np.arange(arr.shape[1]), np.arange(arr.shape[0]))
        indices = (y + dy).reshape(-1), (x + dx).reshape(-1)
        distorted = ndimage.map_coordinates(arr, indices, order=1, mode='reflect').reshape(arr.shape)
...
# 3. OCGAN training function with ElasticTransform
