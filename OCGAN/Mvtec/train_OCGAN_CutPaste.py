...
# CutPaste augmentation class
class CutPaste:
    def __init__(self, area_ratio=0.2, aspect_ratio=(0.3, 3.3), probability=0.5):
    ...
    def __call__(self, img: Image.Image) -> Image.Image:
        if random.random() > self.probability:
            return img
        w, h = img.size
        area = w * h * self.area_ratio
        ar = random.uniform(*self.aspect_ratio)
        cut_w = int(round(np.sqrt(area * ar)))
        cut_h = int(round(np.sqrt(area / ar)))
        x1 = random.randint(0, w - cut_w)
        y1 = random.randint(0, h - cut_h)
        patch = img.crop((x1, y1, x1 + cut_w, y1 + cut_h))
        x2 = random.randint(0, w - cut_w)
        y2 = random.randint(0, h - cut_h)
...
# 3. OCGAN training function with CutPaste
def train_ocgan_mvtec(root, epochs=20, batch_size=32, lr=2e-4):
    # Compose CutPaste + normalization
    train_tf = transforms.Compose([
        CutPaste(area_ratio=0.2, aspect_ratio=(0.3,3.3), probability=0.5),
