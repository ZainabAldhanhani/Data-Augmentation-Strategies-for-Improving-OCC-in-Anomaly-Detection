...
class SimulatedEnvironmentalEffects:
    def __init__(self, noise_std=0.02, fog_prob=0.5):
    ...
    def __call__(self, img: Image.Image) -> Image.Image:
        ...
        # Simulate fog by blending with a white overlay
        if random.random() < self.fog_prob:
            fog = np.ones_like(arr) * 0.8
            alpha = random.uniform(0.2, 0.5)
            arr = np.clip(arr * (1 - alpha) + fog * alpha, 0, 1)
...
# 4. Training loop for Ped1 with Simulated Environmental Effects
def train_ocgan_ped1(root, epochs=20, batch_size=32, lr=2e-4):
    gt_list = load_ucsd_gt(root)
    train_tf = transforms.Compose([
        SimulatedEnvironmentalEffects(noise_std=0.02, fog_prob=0.5),
        default_transform
    ])
