...
class CutPaste:
    def __init__(self, patch_size_ratio=0.2, probability=0.5):
    ...
    def __call__(self, img: Image.Image) -> Image.Image:
        ...
        x1 = random.randint(0, w - ps)
        y1 = random.randint(0, h - ps)
        patch = arr[y1:y1+ps, x1:x1+ps].copy()
        x2 = random.randint(0, w - ps)
        y2 = random.randint(0, h - ps)
        arr[y2:y2+ps, x2:x2+ps] = patch
